...
public class QuizActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (list == null || list.isEmpty()) {
            Log.e("QuizActivity", "Veritabanında hiç soru bulunamadı!");
            return;
        }
        Collections.shuffle(list);
        if (list.size() > sizeofQuiz) {
            list = list.subList(0, sizeofQuiz); // Sadece ilk sizeofQuiz soruyu al
        }
        currentQuestion = list.get(qid);

        // İlk soru numarasını doğru göster
    ...
    private void finalResult(){
        ...
        Intent resultIntent = new Intent(QuizActivity.this, com.nexis.kpss_3.Result.class);
